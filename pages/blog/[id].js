...
export default function EditBlog() {
  ...
  async function fetchBlog() {
    setFetching(true);
    try {
      const result = await fetchFromBackend(`blogs/${id}`);
      const blog = result?.data?.blog ?? result?.data ?? {};
      setForm({
        title: blog.title || "",
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      await fetchFromBackend(`blogs/${id}`, {
        method: "PUT",
        body: JSON.stringify(result.data),
      });
      Swal.fire({ title: "Success", text: "Blog updated successfully.", icon: "success" });
